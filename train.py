...
def inference(images, is_train):
    ...
    fc6 = fc_layer(pool5, 4096, 'fc6'. is_train)
    if is_train:
        fc6 = tf.nn.dropout(fc6, 0.5)
    
    fc7 = fc_layer(fc6, 4096, 'fc7', is_train)
    if is_train:
        fc7 = tf.nn.dropout(fc7, 0.5)
    
    logits = final_fc_layer(fc7, 20, 'fc8', is_train)
    return logits 
    '''
    if is_train:
        return logits
...
def fc_layer(input_op, fan_out, name, is_train):
    '''
    input_op: 输入tensor
    fan_in: 输入节点数
    fan_out： 输出节点数
...
def final_fc_layer(input_op, fan_out, name, is_train):
    with tf.variable_scope(name) as scope:
        ...
        if is_train:
            ...
            conv = tf.nn.conv2d(input_op, kernels_reshape)
            pre_activation = tf.nn.bias_add(conv, biases)
...
def get_conv_kernel(name):
    ...
    var = tf.get_variable(name='kernel', shape=shape, dtype=tf.float32, initializer=init)

    if not tf.get_variable_scope().reuse:
        weight_decay = tf.multiply(tf.nn.l2_loss(var), wl, name='weight_loss')
        tf.add_to_collection("losses", weight_decay)
    #print name, shape, data_dict[name][0]
    return var

def get_fc_weight(name):
    init = tf.constant_initializer(value=data_dict[name][0], dtype=tf.float32)
    shape = data_dict[name][0].shape
    var = tf.get_variable(name='weights', shape=shape, dtype=tf.float32, initializer=init)
...
def get_bias(name):
    ...
    biases = tf.get_variable(name='biases', shape=shape, dtype=tf.float32, initializer=init)
    #print name, shape, data_dict[name][1]
    return biases
...
def get_fc_weight_reshape(name, shape, num_classes=None):
    ...
    init = tf.constant_initializer(value=weights,
                                    dtype=tf.float32)
    var = tf.get_variable(name="weights", shape=shape, dtype=tf.float32, initializer=init)
    print name, shape, weights, weights.shape
    return var
    ...
    for step in range(config.max_step):
        ...
        if (step % 50 == 0) and step:
            ...
            for i in range(num_iter):
                ...
                with tf.device("/gpu:0"):
                    accuracy, predictionval = sess.run([top_k_op, predictions], feed_dict={
            ...
            print predictionval
            print labels_val
            print 'precision of training @ 1 = %.3f' % precision
        '''    
